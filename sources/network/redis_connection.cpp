...
{
  _CPP_REDIS_LOG(debug, "cpp_redis::network::redis_connection created");
}

redis_connection::~redis_connection(void) {
  _CPP_REDIS_LOG(debug, "cpp_redis::network::redis_connection destroyed");
}
...
{
  _CPP_REDIS_LOG(debug, "cpp_redis::network::redis_connection attempts to connect");
  ...
  _CPP_REDIS_LOG(debug, "cpp_redis::network::redis_connection connected");
...
redis_connection::disconnect(void) {
  _CPP_REDIS_LOG(debug, "cpp_redis::network::redis_connection attempts to disconnect");
  m_client.disconnect();
  _CPP_REDIS_LOG(debug, "cpp_redis::network::redis_connection disconnected");
}
...
redis_connection::send(const std::vector<std::string>& redis_cmd) {
  ...
  _CPP_REDIS_LOG(debug, "cpp_redis::network::redis_connection stored new command in the send buffer");
...
redis_connection::commit(void) {
  ...
  _CPP_REDIS_LOG(debug, "cpp_redis::network::redis_connection attempts to send pipelined commands");
  std::string buffer = std::move(m_buffer);
  m_client.send(buffer);
  _CPP_REDIS_LOG(debug, "cpp_redis::network::redis_connection sent pipelined commands");
...
redis_connection::tcp_client_receive_handler(network::tcp_client&, const std::vector<char>& buffer) {
  try {
    _CPP_REDIS_LOG(debug, "cpp_redis::network::redis_connection receives packet, attempts to build reply");
    m_builder << std::string(buffer.begin(), buffer.end());
  }
  catch (const redis_error& e) {
    _CPP_REDIS_LOG(error, "cpp_redis::network::redis_connection could not build reply (invalid format), disconnecting");

    if (m_disconnection_handler) {
      _CPP_REDIS_LOG(debug, "cpp_redis::network::redis_connection calls disconnection handler");
  ...
  while (m_builder.reply_available()) {
    _CPP_REDIS_LOG(debug, "cpp_redis::network::redis_connection reply fully built");

    auto reply = m_builder.get_front();
    m_builder.pop_front();

    if (m_reply_callback) {
      _CPP_REDIS_LOG(debug, "cpp_redis::network::redis_connection executes reply callback");
...
redis_connection::tcp_client_disconnection_handler(network::tcp_client&) {
  _CPP_REDIS_LOG(debug, "cpp_redis::network::redis_connection has been disconnected");

  if (m_disconnection_handler) {
    _CPP_REDIS_LOG(info, "cpp_redis::network::redis_connection calls disconnection handler");
    m_disconnection_handler(*this);
  }
}
