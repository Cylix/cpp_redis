...
redis_client::redis_client(void) {
  _CPP_REDIS_LOG(debug, "cpp_redis::redis_client created");
}

redis_client::~redis_client(void) {
  _CPP_REDIS_LOG(debug, "cpp_redis::redis_client destroyed");
}
...
{
  _CPP_REDIS_LOG(debug, "cpp_redis::redis_client attempts to connect");
  ...
  _CPP_REDIS_LOG(info, "cpp_redis::redis_client connected");
...
redis_client::disconnect(void) {
  _CPP_REDIS_LOG(debug, "cpp_redis::redis_client attempts to disconnect");
  m_client.disconnect();
  _CPP_REDIS_LOG(info, "cpp_redis::redis_client disconnected");
}
...
redis_client::send(const std::vector<std::string>& redis_cmd, const reply_callback_t& callback) {
  ...
  _CPP_REDIS_LOG(info, "cpp_redis::redis_client attemps to store new command in the send buffer");
  m_client.send(redis_cmd);
  m_callbacks.push(callback);
  _CPP_REDIS_LOG(info, "cpp_redis::redis_client stored new command in the send buffer");
...
redis_client::sync_commit(void) {
  ...
  _CPP_REDIS_LOG(debug, "cpp_redis::redis_client finished to wait for callbacks completion");
...
redis_client::try_commit(void) {
  try {
    _CPP_REDIS_LOG(debug, "cpp_redis::redis_client attempts to send pipelined commands");
    m_client.commit();
    _CPP_REDIS_LOG(info, "cpp_redis::redis_client sent pipelined commands");
  }
  catch (const cpp_redis::redis_error& e) {
    _CPP_REDIS_LOG(error, "cpp_redis::redis_client could not send pipelined commands");
    clear_callbacks();
    throw e;
  }
}
...
redis_client::connection_receive_handler(network::redis_connection&, reply& reply) {
  ...
  _CPP_REDIS_LOG(info, "cpp_redis::redis_client received reply");
  {
  ...
  if (callback) {
    _CPP_REDIS_LOG(debug, "cpp_redis::redis_client executes reply callback");
    callback(reply);
  }
...
redis_client::call_disconnection_handler(void) {
  if (m_disconnection_handler) {
    _CPP_REDIS_LOG(info, "cpp_redis::redis_client calls disconnection handler");
...
redis_client::connection_disconnection_handler(network::redis_connection&) {
  _CPP_REDIS_LOG(warn, "cpp_redis::redis_client has been disconnected");
  clear_callbacks();
